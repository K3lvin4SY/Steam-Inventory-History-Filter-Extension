...
function ShowPromptDialog( strTitle, strDescription, strOKButton, strCancelButton, rgModalParams, defaultValue )
{
	...
	var $Body = $('<form/>');
	var $Input = $('<input/>', {type: 'text', 'class': '' } ).val( defaultValue );
	if ( rgModalParams && rgModalParams.inputMaxSize )
	{
		$Input.attr( 'maxlength', rgModalParams.inputMaxSize );
	}
	$Body.append( $('<div/>', {'class': 'newmodal_prompt_description' } ).append( strDescription ) );
	$Body.append( $('<div/>', {'class': 'newmodal_prompt_input gray_bevel for_text_input fullwidth' } ).append( $Input ) );
	...
	var elButtonLabel = $( '<span/>' ).text( strOKButton );
	var $OKButton = $('<button/>', {type: 'submit', 'class': 'btn_green_white_innerfade btn_medium' } ).append( elButtonLabel );
	$OKButton.click( fnOK );
	var $CancelButton = _BuildDialogButton( strCancelButton );
...
function ShowPromptWithTextAreaDialog( strTitle, strInitialText, strOKButton, strCancelButton, textAreaMaxLength, strDescription )
{
	...
	var $Body = $('<form/>');
	var $TextArea = $('<textarea/>', { 'class': 'newmodal_prompt_textarea' } );
	...
	if ( strDescription )
	{
		$Body.append( $('<div/>', {'class': 'newmodal_prompt_with_textarea newmodal_prompt_description' } ).text( strDescription ) );
	}

	$Body.append( $('<div/>', {'class': 'newmodal_prompt_with_textarea gray_bevel fullwidth' } ).append( $TextArea ) );
...
function ShowEditablePrompt( strTitle, obj, onOk, onCancel)
{
	...
	var $Body = $('<form/>');
	var $TextArea = $('<textarea/>', {'class': 'newmodal_prompt_textarea', 'id': 'json_window'});
	...
	var editButtonLabel = $( '<span/>' ).text( strEditButton );
	var $EditButton = $('<button/>', {type: 'button', 'class': 'btn_darkred_white_innerfade btn_medium' } ).append( editButtonLabel );
...
function ShowBlockingWaitDialog( strTitle, strDescription )
{
	...
	var container = $('<div/>', {'class': 'waiting_dialog_container'} );
	var throbber = $('<div/>', {'class': 'waiting_dialog_throbber'} );
...
function _BindOnEnterKeyPressForDialog( Modal, deferred, fnOnEnter )
{
	...
	$(document).on( 'keyup.SharedConfirmDialog', fnOnKeyUp );
	deferred.always( function() { $(document).off( 'keyup.SharedConfirmDialog' ); } );
}
...
{
	var $Dialog = $('<div/>', {'class': 'newmodal'} );
	var $CloseButton = $('<div/>', {'class': 'newmodal_close', 'data-panel': '{"focusable":true,"clickOnActivate":true}' } );
	var $Header = ( $('<div/>', {'class': 'newmodal_header' }) );
	var $TopBar = ( $('<div/>', {'class': 'modal_top_bar' }) );
	if ( strTitle )
		$Header.append( $CloseButton ).append( $('<div/>', {'class': 'title_text' } ).text( strTitle ) );
    if ( rgModalParams && rgModalParams.strSubTitle ) {
        var $SubTitle = (rgModalParams.strSubTitle);
        $Header.append( $('<div/>', {'class': 'subtitle_text' } ).text( $SubTitle ) )
    }
	$Header = $('<div/>', {'class': 'newmodal_header_border'}).append( $Header );
	$Dialog.append( $TopBar ).append( $Header );
	var $Content = $('<div/>', {'class': 'newmodal_content' } );
	$Content.append( $('<div/>').append( strDescription ) );

	if ( rgButtons.length > 0 )
	{
		var $Buttons = $('<div/>', {'class': 'newmodal_buttons', 'data-panel': '{"flow-children":"row"}' } );
	...
	$Dialog.append( $('<div/>', {'class': 'newmodal_content_border' } ).append( $Content ) );
...
function _BuildDialogButton( strText, bActive, rgOptions )
{
	...
	var elButtonLabel = $( '<span/>' ).html( strText );
	var elButton = $('<div/>', {'class': strClassName } ).append( elButtonLabel );
	return elButton;
}
...
{
		var $Dialog = $('<div/>').append( $('<div/>', {'style': 'display:flex'} ).append( content ) );
...
function CModal( $Content, rgParams )
{
	...
	this.m_nInitialOffsetTop = $(window).scrollTop();
	this.m_nInitialOffsetLeft = $(window).scrollLeft();
	...
	$(document.body).append( this.m_$Content );
}
...
CModal.prototype.AdjustSizing = function( duration )
{
	...
	var nViewportWidth = document.compatMode === 'BackCompat' ? document.body.clientWidth : $(window).width();
	var nViewportHeight = document.compatMode === 'BackCompat' ? document.body.clientHeight : $(window).height();
...
CModal.prototype.Show = function()
{
	...
	{
		$(window).on( 'resize', null, this.m_fnSizing );
	}
	CModal.s_$Background.on( 'click.CModal', this.m_fnBackgroundClick );
	$(document).on( 'keyup.CModal', this.m_fnOnEscapeKeyPress );
...
CModal.prototype.Dismiss = function()
{
	...
	if ( !this.m_bIgnoreResizeEvents )
	{
		$(window).off( 'resize', null, this.m_fnSizing );
	}
	...
	if ( !CModal.s_rgModalStack.length )
	{
		...
		$(document).off( 'keyup.CModal', this.m_fnOnEscapeKeyPress );
...
CModal.ShowModalBackground = function()
{
	...
	{
		CModal.s_$Background = $('<div/>', {'class': 'newmodal_background'});
		CModal.s_$Background.css( 'opacity', 0 );
		$(document.body).append( CModal.s_$Background );
	}
...
function InitCookiePreferencesPopup()
{
	...
	if ( $popupDialog.length == 0 )
	{
		...
		$('body').append( $CPopupContent );

		timerCookiePopup = window.setTimeout( function() {
			$("#cookiePrefPopup").show();
...
{
	var $Hover = $('<div/>', {'class': 'miniprofile_hover'} );
	var $HoverContent = $('<div/>', {'class': 'miniprofile_hover_inner shadow_content'} );

	$Hover.append( $('<div class="shadow_ul"></div><div class="shadow_top"></div><div class="shadow_ur"></div><div class="shadow_left"></div><div class="shadow_right"></div><div class="shadow_bl"></div><div class="shadow_bottom"></div><div class="shadow_br"></div>'), $HoverContent );
...
function BindAJAXHovers( $Hover, $HoverContent, oParams )
{
	...
	$(document ).on( 'mouseenter.' + strEventNamespace + ' touchend.' + strEventNamespace, strSelector, function(e) {
		var $Target = $(this);
	...
	$(document ).on( 'click.' + strEventNamespace + ' mouseleave.' + strEventNamespace, strSelector, function(e) {
		var $Target = $(this);
	...
	$(document ).on( 'click', null, fnCancelHover );
...
function PositionMiniprofileHover( $Hover, $Target, oParams )
{
	...
	if ( !$HoverBox.length )
		$HoverBox = $( $Hover.children()[0] );

	var nWindowScrollTop = $(window).scrollTop();
	var nWindowScrollLeft = $(window).scrollLeft();
...
{
	var $Hover = $('<div/>', {'class': 'emoticon_hover'} );
	var $HoverContent = $('<div/>', {'class': 'emoticon_hover_content'} );
	$Hover.append( $HoverContent, $('<div/>', {'class': 'hover_arrow left emoticon_hover_arrow' } ), $('<div/>', {'class': 'hover_arrow right emoticon_hover_arrow' } ) );
	$Hover.hide();
...
function DynamicLink_PlayYouTubeVideoInline( elem, videoid )
{
	...
	var wrapper = $( '<div/>', { 'class': 'dynamiclink_youtubeviewvideo' } );
	var embed_wrapper = $( '<div/>', { 'class' : 'dynamiclink_youtubeviewvideoembedded', 'frameborder' : '0' } );
	var iframeContent = $( '<iframe/>', { 'frameborder' : '0', src: youtubeurl, 'allowfullscreen': '' } );
	if ( el.length )
	{
...
function DynamicLink_PlayVimeoVideoInline( elem, videoid )
{
	...
	var wrapper = $( '<div/>', { 'class': 'dynamiclink_youtubeviewvideo' } );
	var embed_wrapper = $( '<div/>', { 'class' : 'dynamiclink_youtubeviewvideoembedded', 'frameborder' : '0' } );
	var iframeContent = $( '<iframe/>', { 'frameborder' : '0', src: videourl, webkitallowfullscreen : '', mozallowfullscreen : '', allowfullscreen : '' } );
	if ( el.length )
	{
		embed_wrapper.append( iframeContent );
		wrapper.append( embed_wrapper );
...
function DynamicLink_ShowSketchfabModelInline( elem, details )
{
	...
	var el = $(elem);
	var wrapper = $( '<div/>', { 'class' : 'dynamiclink_sketchfabmodelembedded', 'frameborder' : '0' } );
	var iframeContent = $( '<iframe/>', { 'frameborder' : '0', "mozallowfullscreen" : true, "webkitallowfullscreen" : true, src: url } );
	if ( el.length )
	{
...
function ShowBannedDynamicLink( el )
{
	...
	if ( el.length > 0 )
	{
		...
		var link = $( "<a>", { href: bannedContentURL, text: bannedContentURL } );
		el.html( link );
		el.attr( 'onclick', '' );
	}
}
...
CScrollOffsetWatcher.RegisterWatcher = function( Watcher )
{
	...
	if ( !bHadWatchers )
	{
		$(window).on( 'scroll.ScrollOffsetWatcher', CScrollOffsetWatcher.OnScroll );

		var nRecalcTimer = 0;
		$(window).on( 'resize.ScrollOffsetWatcher', function() {
			if ( nRecalcTimer )
...
CScrollOffsetWatcher.OnScroll = function()
{
	...
	{
		$(window).off( 'scroll.ScrollOffsetWatcher' );
		$(window).off( 'resize.ScrollOffsetWatcher' );
	}
};

function LoadImageGroupOnScroll( elTarget, strGroup )
{
	var $Element = $( '#' + elTarget );
	if ( $Element.length )
...
function LoadDelayedImages( group )
{
	if ( typeof g_rgDelayedLoadImages != 'undefined' && g_rgDelayedLoadImages[group] )
	{
		...
		{
			$('#delayedimage_' + group + '_' + i).attr( 'src', rgURLs[i] );
		}
...
{
	if ( bVoteUp && $('#VoteUpBtn_' + gid).hasClass( "btn_active" ) )
	{
		return;
	}
	if ( !bVoteUp && $('#VoteDownBtn_' + gid).hasClass( "btn_active" ) )
	{
		return;
	}
	...
	$.post( rateURL, {
		...
		var votesUpCount = $('#VotesUpCount_' + gid);
		if ( votesUpCount )
		{
			var increment = 0;
			if ( bVoteUp )
			...
			else if ( $('#VoteUpBtn_' + gid).hasClass( 'btn_active' ) )
			{
				increment = -1;
			}
			...
			{
				$('#VotesUpCountContainer_' + gid).hide();
			}
			else
			{
				$('#VotesUpCountContainer_' + gid).show();
			}
		}

		if ( bVoteUp )
		{
			$('#VoteUpBtn_' + gid).addClass( "btn_active" );
			$('#VoteDownBtn_' + gid).removeClass( "btn_active" );
		}
		else
		{
			$('#VoteDownBtn_' + gid).addClass( "btn_active" );
			$('#VoteUpBtn_' + gid).removeClass( "btn_active" );
		}
...
function GetResponsiveHeaderFixedOffsetAdjustment()
{
	...
	var $ResponsiveHeader = $('.responsive_header:visible');
...
function ScrollToIfNotInView( elem, nRequiredPixelsToShow, nSpacingBefore, nAnimationSpeed )
{
	...
	var nViewportOffsetTop = elemTop - $(window).scrollTop();
	...
	else
	{
		...
		if ( nViewportOffsetBottom > $(window).height() )
			bNeedToScroll = true;
	}
	...
	if ( bNeedToScroll )
	{
		...
		if ( typeof nAnimationSpeed != 'undefined' )
			$('html, body' ).animate( {scrollTop: nViewportOffsetTop}, nAnimationSpeed );
		else
...
function CAjaxInfiniteScrollingControls( rgSearchData, url )
{
	...
	$(document).scroll( this.m_fnRawScrollHandler );

	// If we have a _scroll_top element, then set our OnUnload handler.
	if ( $( "#" + this.m_strElementPrefix + '_scroll_top').length )
...
CAjaxInfiniteScrollingControls.prototype.Stop = function()
{
	...
	$(document).off( "scroll", this.m_fnRawScrollHandler );
...
CAjaxInfiniteScrollingControls.prototype.RestoreScrollTop = function( bForce )
{
	...
	var scrollTopPrevious = parseInt( $( "#" + this.m_strElementPrefix + '_scroll_top').val() );
...
CAjaxInfiniteScrollingControls.prototype.OnUnload = function()
{
	...
	$( "#" + this.m_strElementPrefix + '_scroll_top').val( scrollTop );
};
...
function CSlider( $Container, $Grabber, args )
{
	...
	this.m_$Container.on( 'mousedown touchstart', function( event ) {
		...
		$(document).on( 'mousemove.CSlider touchmove.CSlider', function( event ) {
		...
		$(document).on( 'mouseup.CSlider touchend.CSlider', function( event ) {
			$(document).off('.CSlider');
...
function CScrollSlider( $Scroll, $Container, $Grabber, args )
{
	...
	this.m_Slider = new CSlider( $Slider.length ? $Slider : $Container, $Grabber, { fnOnChange: $.proxy( this.OnSliderChange, this )} );
	...
	$(window).on('resize.ScrollSlider', function() {
		_this.UpdateRanges();
	} );
...
{
	var $Ignore = $FromIDOrElement( elemIgnore );
	// delay registration by one frame so that we don't catch the event that triggered this popup.
	window.setTimeout( function() {
		$(document).on('click.RegisterPopupDismissal keydown.RegisterPopupDismissal', function (event) {
			...
			if ( $Ignore.length && $.contains( $Ignore[0], elem[0] ) )
				return;
...
{
	$(document).off( '.RegisterPopupDismissal' );

	if ( elemIgnore )
	{
		// unregister gamepad B button press handling
		$FromIDOrElement( elemIgnore ).off( 'vgp_oncancel' );
	}
}
...
{
	var $Popup = $FromIDOrElement(elemPopup);
	...
	var $Link = $FromIDOrElement(elemLink);
...
function HideFlyoutMenu( event, elemLink, elemPopup )
{
	...
	if ( event )
	{
		...
		if ( !reltarget.length ||
			( $Link.length && $.contains( $Link[0], reltarget[0] ) ) ||
			( $Popup.length && $.contains( $Popup[0], reltarget[0] ) ) ||
			$Link.is( reltarget ) )
			return;
	}
...
function AlignMenu( elemLink, elemPopup, align, valign, bLinkHasBorder, elemAlternateAlignTo )
{
	...
	var $LinkVertical = $FromIDOrElement(elemLink);
	var $LinkHorizontal = elemAlternateAlignTo ? $FromIDOrElement(elemAlternateAlignTo) : $LinkVertical;
...
function BindAutoFlyoutEvents()
{
	var fnShowFlyout = function( $Tab, bIsHover, bTakeFocus )
	{
		...
		if ( !$Content.data('flyout-events-bound') )
		{
			$Content.on('mouseleave.Flyout', function( e ) {
				...
				if ( $Tab.is( e.relatedTarget ) || $.contains( $Tab[0], e.relatedTarget ) )
					return;
			...
			$Content.on('vgp_onblur', function( e ) {
				...
				console.log( node );
				if ( !node || !$.contains( e.currentTarget, node.Element ) )
					HideFlyoutMenu( null, $Tab, $Content );
			});
		...
		{
			window.setTimeout( function() {
				$(document).on('click.FlyoutDismiss', function(e) {
					if ( $.contains( $Content[0], e.target ) || $Content.is( e.target ) )
						return;
	...
	$(document).on( 'mouseenter.Flyout click.Flyout', '.flyout_tab', function(e) {
		var $Tab = $(this);
	...
	$(document).on('mouseleave.Flyout', '.flyout_tab', function(e) {
		var $Tab = $(this);
		var $Content = $('#' + $Tab.data('flyout') );
		var bResponsiveSlidedownMenu = UseSmallScreenMenu() && $Content.hasClass('responsive_slidedown');

		if ( !$Content.length || $Content.data('flyout-event-running') || bResponsiveSlidedownMenu ||
			$Content.is( e.relatedTarget ) || $.contains( $Content[0], e.relatedTarget ) )
			return;
		...
		$(document).on('vgp_onok', '.flyout_tab', function( e ) {
		fnShowFlyout( $(this), false, true );
		e.stopPropagation();
	});
}
...
function PollOnUserActionAfterInterval( strNamespace, nPollInterval, fnCallback, nPollImmediateInterval )
{
	...
	fnInvokeCallbackOnNextUserAction = function() {
		window.setTimeout( function() {
			...
			if ( nPollImmediateInterval && $.now() <= tsLastUserAction + nPollImmediateInterval )
			{
				fnDoPoll();
			}
			else
			{
				$(window).on( strEvents, function() {
					$(window ).off( '.' + strNamespace );
					tsLastUserAction = $.now();
					fnDoPoll();
				});
			}
		}, nPollInterval );
...
{
	var $NotificationItems = $('.notification_ctn');
	var $NotificationTotalCounts = $('.notification_count_total_ctn');
...
function UpdateNotificationCounts()
{
	...
	if ( $NotificationItems.length || $NotificationTotalCounts.length )
	{
		$.ajax( GetDefaultCommunityAJAXParams( 'actions/GetNotificationCounts', 'GET' ) ).done( function( data ) {
			...
			if ( notifications )
			{
				...
				if ( !notifications[ 11 ] )
				{
					$('.global_header_account_alert').hide();
				}
...
function PostToURLWithSession( url, rgParams )
{
	...
	if ( typeof g_sessionID != 'undefined' )
		$Form.append( $('<input/>', {'type': 'hidden', 'name': 'sessionid', 'value': g_sessionID } ) );

	if ( rgParams )
	{
		for ( var name in rgParams )
			$Form.append( $('<input/>', {'type': 'hidden', 'name': name, 'value': rgParams[name] } ) );
	}
	$Form.appendTo( 'body' );
	$Form.submit();
}
...
window.VScrollbar = function( eleTarget, eleHandle, direction )
{
	...
	$(eleTarget).bind('mousewheel DOMMouseScroll',function( event ){
	...
	$(eleHandle.parent()).bind('click',function( event ){
	...
	eleHandle.mousedown(function( event ){
		$(window).bind('mousemove.scroll', funcMouseMove);
		event.stopPropagation();
	});
...
function CAutoComplete( elInput, fnSearch, fnOnChange )
{
	...
	this.m_$Popup = $ ( '<div class="' + strName + '"></div>' );
...
CAutoComplete.prototype.SetPopupContents = function( $Contents )
{
	...
	var _this = this;
	this.m_$Popup.children().on( 'click.autocomplete', function( event ) { _this.SelectSuggestion( $( this ) ); } );
	this.m_$Popup.show();
}
...
function FixedElementOnScrollWrapper ( elemID, fixedOffsetTop, bScrollWithPageIfTooTall, docHeightOffset, params )
{
	...
	this.$FixedElement = $FromIDOrElement( elemID );
	...
	this.$FixedElementPadding = $('<div/>', {'class': 'FixedElementOnScrollWrapper_padding', 'id': 'ScrollWrapperPadding_' + elemID } ).hide();
	this.fixedElement.insert( { before: this.$FixedElementPadding[0] } );

	var _this = this;
	$(window).on( 'scroll.FixedElementOnScrollWrapper hashchange.FixedElementOnScrollWrapper', function() { _this.handleScroll() } );
	$(window).on( 'resize.FixedElementOnScrollWrapper', function() { _this.handleScroll( true /* force recalc */ ) } );
	this.handleScroll();
}
...
CTextInputSuggest.prototype.Init = function( $InputElement, fnSuggestForTerm, fnOnSuggest, strCssClass )
{
	...
	this.m_$SuggestionsCtn = $('<div/>', {'class': strCssClass, style: 'display: none;' } );
	this.m_$Suggestions = $('<div/>', {'class': 'popup_body popup_menu' } );
	this.m_$SuggestionsCtn.append( this.m_$Suggestions );

	this.m_$SuggestionsCtn.hide();
	$(document.body).append( this.m_$SuggestionsCtn );

	var zIndex = 200;	//normal popup zindex
	this.m_$Input.parents().each( function() {
		var zIndexParent = $(this).css('zIndex');
...
CTextInputSuggest.prototype.SetSuggestions = function( rgSuggestions )
{
	...
	if ( rgSuggestions && rgSuggestions.length )
	{
		...
		{
			var $Suggestion = $('<div/>', {'class': 'suggestion_item popup_menu_item' } );
			$Suggestion.text( rgSuggestions[i] );
			$Suggestion.click( $.proxy( this.OnSuggestionSelected, this, $Suggestion ) );
			$Suggestion.mouseenter( $.proxy( this.SetFocus, this, $Suggestion ) );
...
var CAjaxSubPageController = function( elTarget, strBaseURL, strInstanceId, strDefaultLocation )
{
	...
	var _this = this;
	setTimeout( function(){ $(_this.elTarget).children().trigger('load'); }, 1);
...
CAjaxSubPageController.prototype.InstrumentLinks = function( elTarget )
{
	...
	var rgLinks = $('[data-'+this.strStateID+']', elTarget);
...
CAjaxSubPageController.prototype.Navigate = function( strLocation, strPageTitle, event )
{
	...
	$( 'form', this.elTarget ).trigger('saveform');
	...
	$.ajax({
		...
		var elNewContents = $( result );

		$(_this.elTarget).empty();
		$(_this.elTarget).append( elNewContents );


		_this.elTarget.classList.remove('loading');

		$( elNewContents ).trigger('load');

		_this.InstrumentLinks( _this.elTarget );
		if( strPageTitle )
			document.title = strPageTitle;

		$('.tooltip', _this.elTarget).v_tooltip();
...
CAjaxSubPageController.prototype.OnWindowPopState = function( event )
{
	...
	if( state && state.id == this.strStateID)
	{
		...
		var elNewContents = $(state.html);
		$(this.elTarget).empty();
		$(this.elTarget).append(elNewContents);
		if( state.title )
...
{
	var $Images = $( '[data-animate-on-hover-src]', document );
	if ( $Images.length )
	{
...
{
	// Standard tooltips
	var $TextTooltips = $( '[data-tooltip-text]', selector);
	...
	var $HTMLTooltips = $( '[data-tooltip-html]', selector);
...
function SetupTooltips( rgOptions )
{
    ...
    InnerSetupTooltips();
    $( window ).on( 'Responsive_SmallScreenModeToggled', InnerSetupTooltips );
}
...
function ViewTitlesWithDescriptors( descid )
{
	...
	$.get( 'https://store.steampowered.com/search/results/',
		...
		var content = $( "<div>" );
		content.append( $( "<div>", { class: 'content_descriptors_examples_desc', text: response.desc } ) );
		...
		if ( response.items.length != 0 )
		{
			for ( var i = 0; i < response.items.length && i < 10; ++i )
			{
				...
				var elem = $( "<div>", { class: 'content_descriptors_example_app' } );
				elem.append( $( "<img>", {  class: 'app_logo', src: item.logo } ) );
				elem.append( $( "<div>", { class: 'app_name', text: item.name } ) );
				content.append( elem );
			}
		}
		else
		{
			content.append( $( "<div>", { class: "no_items", text: 'No Products Found' } ) );
		}
...
{
	if( $(elTarget).data('manual-tracking' ) )
		return;
...
{
	window.history.replaceState( $.extend( {}, window.history.state, { notify_focus_restore_ready: true } ), "" );
...
{
	$( 'body' ).toggleClass( 'OverlayWindowPinned', bPinned );
	$( 'body' ).toggleClass( 'OverlayWindowPinnedView', bShowPinnedView );
}
